...
import com.github.beetv.ext.resource
...
object MockData {
    ...
    fun randomItems() = mockItems.shuffled().take(Random.nextInt(4, mockItems.size))

    val mockChannels = listOf(
        Channel(
            2,
            resource(R.drawable.ic_movie),
            "美剧TV",
            ChannelType.SPIDER,
            ViewStyle.DEFAULT,
            config = SpiderChannelConfig("classes.dex", "com.github.beetv.spider.MeiJuTVSpider")
        ),
        Channel(
            3,
            resource(R.drawable.ic_alipan),
            "阿里云盘1",
            ChannelType.ALI_PAN,
        ),
        Channel(
            4,
            resource(R.drawable.ic_sd_storage),
            "本机",
            ChannelType.LOCALHOST,
        ),
        Channel(
            5,
            resource(R.drawable.ic_alist),
            "小雅AList",
            ChannelType.A_LIST,
            FileExplorerViewStyle()
        ),
    )
}
